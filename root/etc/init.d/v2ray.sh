...
outbound_section_validate() {
	uci_validate_section "$NAME" "outbound" "$1" \
		...
		'ss_network:or("tcp", "kcp", "ws", "http", "domainsocket", "quic", "grpc")' \
		'ss_security:or("none", "tls")' \
		'ss_tls_server_name:host' \
...
add_v2ray_redirect_rules() {
	...
	if [ -n "$addition" ]; then
		...
		cat <<-EOF >/tmp/redirect_nftables_1.txt
			*mangle
			:V2RAY -
		...
		*mangle
		...
		$(
			if [ -n "$lan_ipaddrs" ]; then
				local ipaddr
				for ipaddr in $lan_ipaddrs; do
					echo "-A V2RAY -p udp --dport 53 -d $ipaddr --on-ip 127.0.0.1 -j TPROXY --on-port $port --tproxy-mark 0x1/0x1"
				done
			fi
		)
		-A V2RAY -p udp -j RETURN -m set --match-set $nfset_dst_direct dst
		$(
				if [ "$addition" = "dns" ]; then
					...
					echo "-A V2RAY -p udp --on-ip 127.0.0.1 -j TPROXY --on-port $port --tproxy-mark 0x1/0x1"
			fi
...
add_v2ray_tproxy_rules() {
	...
	ip rule add fwmark 0x1/0x3 lookup 100
	ip route add local 0.0.0.0/0 dev lo table 100 2>/dev/null

	ip -6 rule add fwmark 0x1/0x3 lookup 100
	ip -6 route add local ::/0 dev lo table 100

	cat <<-EOF >/tmp/tproxy_nftables.txt
		table inet $NFTABLE {
			chain prerouting {
				...
				#ip6 nexthdr {tcp, udp} tproxy ip6 to :$port meta mark set 1
			}
		}
	EOF

	nft -f /tmp/tproxy_nftables.txt
}

clear_v2ray_rules() {
	while ip rule del fwmark 1 lookup 100 2>/dev/null; do true; done
	while ip rule del fwmark 0x1 lookup 100 2>/dev/null; do true; done
	while ip rule del fwmark 0x1/0x3 lookup 100 2>/dev/null; do true; done

	ip route flush table 100
	ip route del local 0.0.0.0/0 dev lo table 100 2>/dev/null

	while ip -6 rule del fwmark 1 lookup 100 2>/dev/null; do true; done
	while ip -6 rule del fwmark 0x1 lookup 100 2>/dev/null; do true; done
	while ip -6 rule del fwmark 0x1/0x3 lookup 100 2>/dev/null; do true; done
...
create_v2ray_nfset() {
	...
	nft -f - <<-EOF 2>/dev/null
		...
		$(echo "$outbound_v6" | grep -v "^$" | sed "s/.*/add element inet $NFTABLE $NFSET_DST_DIRECT_V6 {&}/")
	EOF
}

destroy_v2ray_rules() {
	nft -f - <<-EOF 2>/dev/null
		delete table inet $NFTABLE
	EOF
}
...
init_rules_for_listfile() {
	...
	if [ -n "$direct_content" ]; then
		echo "$direct_content" |
			...
			sed "s/.*/add element inet $NFTABLE $NFSET_DST_DIRECT_V4 {&}/" |
			nft -f - 2>/dev/null

		echo "$direct_content" |
			grep -oE "([0-9a-fA-F]{0,4}:){1,7}([0-9a-fA-F]){0,4}(/[0-9]{1,2})?" |
			sed "s/.*/add element inet $NFTABLE $NFSET_DST_DIRECT_V6 {&}/" |
			nft -f - 2>/dev/null

		if [ -n "$direct_list_dns" ]; then
			echo "$direct_content" |
				grep -oE "([0-9a-zA-Z_-]+\.)+[a-zA-Z]{2,}$" |
				sed "s|.*|server=/&/$direct_list_dns\nnftset=/&/4#inet#$NFTABLE#$NFSET_DST_DIRECT_V4,6#inet#$NFTABLE#$NFSET_DST_DIRECT_V6|" \
				>>"$FILE_V2RAY_DNSMASQ_CACHE"
		else
			echo "$direct_content" |
				grep -oE "([0-9a-zA-Z_-]+\.)+[a-zA-Z]{2,}$" |
				sed "s|.*|nftset=/&/4#inet#$NFTABLE#$NFSET_DST_DIRECT_V4,6#inet#$NFTABLE#$NFSET_DST_DIRECT_V6|" \
				>>"$FILE_V2RAY_DNSMASQ_CACHE"
		fi
	fi
	...
	if [ -n "$proxy_content" ]; then
		echo "$proxy_content" |
			grep -oE "[0-9]{1,3}(\.[0-9]{1,3}){3}(/[0-9]{1,2})?" |
			sed "s/.*/add element inet $NFTABLE $NFSET_DST_PROXY_V4 {&}/" |
			nft -f - 2>/dev/null

		echo "$proxy_content" |
			grep -oE "([0-9a-fA-F]{0,4}:){1,7}([0-9a-fA-F]){0,4}(/[0-9]{1,2})?" |
			sed "s/.*/add element inet $NFTABLE $NFSET_DST_PROXY_V6 {&}/" |
			nft -f - 2>/dev/null

		if [ -n "$proxy_list_dns" ]; then
			echo "$proxy_content" |
				grep -oE "([0-9a-zA-Z_-]+\.)+[a-zA-Z]{2,}$" |
				sed "s|.*|server=/&/$proxy_list_dns\nnftset=/&/4#inet#$NFTABLE#$NFSET_DST_PROXY_V4,6#inet#$NFTABLE#$NFSET_DST_PROXY_V6|" \
				>>"$FILE_V2RAY_DNSMASQ_CACHE"
		else
			echo "$proxy_content" |
				grep -oE "([0-9a-zA-Z_-]+\.)+[a-zA-Z]{2,}$" |
				sed "s|.*|nftset=/&/4#inet#$NFTABLE#$NFSET_DST_PROXY_V4,6#inet#$NFTABLE#$NFSET_DST_PROXY_V6|" \
				>>"$FILE_V2RAY_DNSMASQ_CACHE"
		fi
	fi
  ...
  # For local devices outbound list
	...
	if [ -n "$src_content" ]; then
		echo "$src_content" |
			grep -oE "[0-9]{1,3}(\.[0-9]{1,3}){3}(/[0-9]{1,2})?" |
			sed "s/.*/add element inet $NFTABLE $NFSET_SRC_DIRECT_V4 {&}/" |
			nft -f - 2>/dev/null
	fi
...
add_dns_settings() {
	...
	if [ -n "$hosts" ]; then
		...
		# fixme: multiple ip resolve still fail
		local h
		for h in $hosts; do
...
setup_transparent_proxy() {
	...
	case "${proxy_mode:-default}" in
		"cn_direct")
			...
			if [ -n "$chnroute" ]; then
				nft -f - <<-EOF 2>/dev/null
					$(echo "$chnroute" | sed "s/.*/add element inet $NFTABLE $NFSET_DST_DIRECT_V4 {&}/")
				EOF
			fi

			if [ -n "$chnroute6" ]; then
				nft -f - <<-EOF 2>/dev/null
					$(echo "$chnroute6" | sed "s/.*/add element inet $NFTABLE $NFSET_DST_DIRECT_V6 {&}/")
				EOF
			fi
		...
		"cn_proxy")
			...
			if [ -n "$chnroute" ]; then
				nft -f - <<-EOF 2>/dev/null
					$(echo "$chnroute" | sed "s/.*/add element inet $NFTABLE $NFSET_DST_PROXY_V4 {&}/")
				EOF
			fi

			if [ -n "$chnroute6" ]; then
				nft -f - <<-EOF 2>/dev/null
					$(echo "$chnroute6" | sed "s/.*/add element inet $NFTABLE $NFSET_DST_PROXY_V6 {&}/")
				EOF
			fi

			ext_args="-m set --match-set $NFSET_DST_PROXY_V4 dst"
			;;
		"gfwlist_proxy")
	...
	local lan_devices lan_ipaddrs lan_ipaddrs6
	...
	if [ -n "$lan_ifaces" ]; then
		...
		for lan in $lan_ifaces; do
			local device ipaddrs ipaddr ipaddrs6 ipaddr6
			network_get_device device "$lan"
			network_get_ipaddrs ipaddrs "$lan"
			network_get_ipaddrs6 ipaddrs6 "$lan"
			...
			if [ -n "$ipaddrs6" ]; then
				for ipaddr6 in $ipaddrs6; do
					if [ -n "$lan_ipaddrs6" ]; then
						lan_ipaddrs6="$ipaddr6"
					else
						lan_ipaddrs6="$lan_ipaddrs6 $ipaddr6"
					fi
				done
			fi
		done
	...
	if [ "x$TRANSPARENT_PROXY_USE_TPROXY" = "x1" ]; then
		...
		add_v2ray_tproxy_rules "$ext_args" "$lan_devices" "$lan_ipaddrs" "$lan_ipaddrs6"
	else
		add_v2ray_redirect_rules "$ext_args" "$lan_devices" "$lan_ipaddrs" "$lan_ipaddrs6"
	fi
}
...
start_instance() {
	...
	if [ -n "$config_file" ]; then
		...
		# xray doesn't have test command
...
service_triggers() {
	procd_add_reload_trigger "$NAME" "firewall"
}
